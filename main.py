...
locationOptions = [[[3708.786865, 402.01, 5974.589844, -131.125488], [3710.265137, 402.01, 5973.340820, -158.777390]],
                   [[14353.624023, 3.326771, 13229.217773, 82.998276],
                    [14378.793945, 3.325975, 13271.585938, -156.011154]],
...
def travelling_trader():
    ...
    string = "ExpansionTraderBoris." + chosen_item_list + "|" + str(chosen_location[0][0]) + " " + str(
        chosen_location[0][1]) + " " + str(chosen_location[0][2]) + "|" + str(
        chosen_location[0][3]) + " 0.000000 -0.000000|" + traderGear
    ...
    with open(
            'F:/SteamLibrary/steamapps/common/DayZServer/profile/ExpansionMod/Quests/Objectives/Travel/SHIB-OBJ-2303-Trader.json',
            'r') as json_file:
        data = json.load(json_file)
        data["Position"] = [chosen_location[1][0], chosen_location[1][1], chosen_location[1][2]]
        with open(
